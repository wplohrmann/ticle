...
function Game() {
  ...
  const setIndividualWordState = (i, j, word) => {
    setWordState(wordState.map((row, x) => row.map((cell, y) => x === i && y === j ? word : cell)));
  }
  ...
  const wordles = gameState.map((row, i) => row.map((winner, j) =>
    <Wordle
      correctWord={correctWords[i][j]}
      player={currentPlayer}
      coords={[i, j]}
      wordState={wordState[i][j]}
      isActive={activeGame !== null && activeGame[0] === i && activeGame[1] === j}
      isPopUp={false}
      winner={winner}
  ...
  if (activeGame !== null) {
    ...
    inputWordle = activeGame !== null && <Wordle
      correctWord={correctWords[i][j]}
      player={currentPlayer}
  ...
  return (<>
    ...
    <div style={{position: "relative"}}>
      {activeGame !== null && <PopUp popUpRef={popUpRef}>{inputWordle}</PopUp>}
